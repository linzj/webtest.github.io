...
/* harmony export */   kSampleWindowBytes: () => (/* binding */ kSampleWindowBytes),
/* harmony export */   logger: () => (/* binding */ logger),
/* harmony export */   performanceLog: () => (/* binding */ performanceLog),
/* harmony export */   setQueueSizesForResolution: () => (/* binding */ setQueueSizesForResolution),
/* harmony export */   verboseLog: () => (/* binding */ verboseLog),
/* harmony export */   warnLog: () => (/* binding */ warnLog)
/* harmony export */ });
let kEncodeQueueSize = 23;
let kDecodeQueueSize = 23;

// Compressed sample data is loaded through a sliding window of this size
// instead of the whole processing range, to bound memory on mobile devices.
const kSampleWindowBytes = 32 * 1024 * 1024;

// Memory budget for each of the decoder/encoder frame queues. Decoded frames
// are typically I420 (~1.5 bytes/pixel), so this caps in-flight uncompressed
// frames at ~128MB per queue regardless of resolution. Measured on an 11GB
// Android phone: a 192MB budget (15 frames at 4K) peaks at ~695MB total PSS
// and can still be killed by lmkd under system memory pressure, so keep
// headroom.
const kQueueMemoryBudgetBytes = 128 * 1024 * 1024;
const kMinQueueSize = 4;
const kMaxQueueSize = 23;

/**
 * Adjusts decoder/encoder queue depths so the number of in-flight
 * uncompressed frames stays within a memory budget, instead of using fixed
 * resolution tiers. A 4K I420 frame is ~12MB, so this yields ~10 queued
 * frames at 4K and the maximum of 23 at 1080p and below.
 * @param {number} width - Output frame width in pixels.
 * @param {number} height - Output frame height in pixels.
 */
function setQueueSizesForResolution(width, height) {
  const bytesPerFrame = Math.ceil(width * height * 1.5);
  const size = Math.max(kMinQueueSize, Math.min(kMaxQueueSize, Math.floor(kQueueMemoryBudgetBytes / bytesPerFrame)));
  kEncodeQueueSize = size;
  kDecodeQueueSize = size;
  infoLog("Logging", `Queue sizes for ${width}x${height}: decode=${kDecodeQueueSize}, encode=${kEncodeQueueSize}`);
}
...
class PreviewManager {
  ...
  async preparePreview(percentage) {
    ...
    await this.sampleManager.ensureSampleData(samples[0].number, samples[samples.length - 1].number);
    return timeStamp;
  }
...
class ProcessingPipeline {
   ...
   */
  async setup(config) {
    // Bound the in-flight uncompressed frame count for this resolution
    // before any decode/encode queues are created.
    const {
      width,
      height
    } = this.getEncoderDimensions();
    (0,_logging_js__WEBPACK_IMPORTED_MODULE_0__.setQueueSizesForResolution)(width, height);
   ...
   * Sample data streams in through a sliding window: before handing chunks to
   * the decoder, this waits until the next window of sample data is loaded
   * (and data for already-consumed samples is released). Concurrent dispatch
   * requests are accumulated and serialized.
  ...
  dispatch(n) {
    ...
    this.pendingDispatchCount = (this.pendingDispatchCount || 0) + n;
    ...
    (async () => {
      try {
        while (this.state === "processing" && this.pendingDispatchCount > 0) {
          const count = this.pendingDispatchCount;
          this.pendingDispatchCount = 0;
          (0,_logging_js__WEBPACK_IMPORTED_MODULE_0__.verboseLog)(`Dispatching ${count} chunks`);
          await this.sampleManager.ensureChunkData(count);
          if (this.state !== "processing") {
            break;
          }
          let exhausted = false;
          this.sampleManager.requestChunks(count, chunk => {
        ...
        (0,_logging_js__WEBPACK_IMPORTED_MODULE_0__.errorLog)("ProcessingPipeline", "Dispatch error", error);
      } finally {
  ...
  /**
   * Called when all samples have been dispatched to the decoder.
   * Flushes the decoder, waits for both queues to drain, and finalizes.
   * @private
   */
  async handleExhausted() {
...
class SampleManager {
  ...
  constructor() {
    ...
    this.currentIndex = 0;
    this.sliceStart = 0; // Absolute index of samples[0] within originalSamples
    this.windowBytes = _logging_js__WEBPACK_IMPORTED_MODULE_0__.kSampleWindowBytes; // Sliding window for on-demand loading
   ...
   *   The loader fills the `data` field of the index entries in
   *   [firstSampleNumber, firstSampleNumber + expectedCount) from the raw
   *   file bytes in [byteStart, byteEnd).
   */
  ...
  async ensureSampleData(startIndex, endIndex) {
    ...
    const loadPromise = this.dataLoader(minOffset, maxEnd, first, last - first + 1);
  ...
  /**
   * Finds the inclusive end index such that the byte size of samples in
   * [startIndex, endIndex] covers at least one sliding window. Operates on
   * this.samples (pre-finalize this is the full original index).
   * @param {number} startIndex - The first sample index.
   * @returns {number} - The inclusive end index.
   * @private
   */
  windowEndIndex(startIndex) {
    ...
    for (let i = startIndex; i < this.samples.length; i++) {
      bytes += this.samples[i].size;
      end = i;
      if (bytes >= this.windowBytes) break;
    }
    return end;
  }

  /**
   * Ensures data for the next `count` samples (from currentIndex) is loaded.
   * Only the samples needed right now are awaited; the rest of the sliding
   * window is filled by a background prefetch so the decoder never waits on
   * I/O. Data for samples already consumed (their bytes have been copied
   * into EncodedVideoChunks) is released, keeping compressed-data memory
   * bounded by the window size instead of the whole processing range.
   * @param {number} count - The number of upcoming samples that must be ready.
   * @returns {Promise<void>}
   */
  async ensureChunkData(count) {
    if (this.currentIndex >= this.samples.length) return;
    const absStart = this.sliceStart + this.currentIndex;
    const neededEnd = this.sliceStart + Math.min(this.currentIndex + count, this.samples.length) - 1;

    // Wait only if the samples needed right now are not loaded yet.
    let firstMissing = -1;
    for (let i = absStart; i <= neededEnd; i++) {
      if (!this.originalSamples[i].data) {
        firstMissing = i;
        break;
      }
    ...
    if (firstMissing >= 0) {
      await this.ensureSampleData(firstMissing, neededEnd);
    }

    // Release data for samples already handed to the decoder.
    for (let i = this.sliceStart; i < absStart; i++) {
      const entry = this.originalSamples[i];
      if (entry && entry.data) {
        entry.data = undefined;
      }
    }

    // Fill the rest of the sliding window in the background.
    const windowEnd = this.sliceStart + this.windowEndIndex(this.currentIndex);
    this.prefetch(absStart, windowEnd);
  }

  /**
   * Fills missing sample data in [startIndex, endIndex] (absolute indices)
   * in the background. ensureSampleData serializes overlapping loads via
   * pendingLoad, so this never runs more than one load at a time.
   * @param {number} startIndex - First absolute sample index.
   * @param {number} endIndex - Last absolute sample index (inclusive).
   * @private
   */
  prefetch(startIndex, endIndex) {
    ...
    if (first < 0) return;
    this.ensureSampleData(first, endIndex).catch(e => {
      (0,_logging_js__WEBPACK_IMPORTED_MODULE_0__.warnLog)("SampleManager", "Prefetch failed", e);
    });
  }
  ...
  async finalizeTimeRange(timeRangeStart, timeRangeEnd) {
    ...
    // Load only the first sliding window; the rest streams in via
    // ensureChunkData() as the decoder consumes samples.
    await this.ensureSampleData(startIndex, this.windowEndIndex(startIndex));
...
class MP4Demuxer {
   ...
   * sample index and installs the on-demand data loader (raw byte-range
   * reads, no mp4box extraction).
  ...
  onReady(info) {
    ...
    // Load sample data by reading the raw byte range directly from the
    // source file. The sample index already carries offset/size, so the
    // bytes are exactly what mp4box extraction would return, without any
    // parsing cost. Each entry's data is a view into the window buffer.
    this.sampleManager.setDataLoader(async (byteStart, byteEnd, firstSampleNumber, expectedCount) => {
      const buffer = await this.sourceFile.slice(byteStart, byteEnd).arrayBuffer();
      for (let i = 0; i < expectedCount; i++) {
        const entry = this.sampleManager.originalSamples[firstSampleNumber + i];
        if (entry && !entry.data) {
          entry.data = new Uint8Array(buffer, entry.offset - byteStart, entry.size);
        }
      }
    });
...
class VideoEncoder {
  ...
  async init(width, height, fps, useCalculatedBitrate, useFileSystem = false) {
    ...
    if ((width > maxWidth || height > maxHeight) && false) // removed by dead control flow
{}
...
class VideoProcessor {
  ...
  async processFileByTime(startMs, endMs) {
    ...
    [this.nb_samples, this.timeRangeStart, this.timeRangeEnd] = await this.sampleManager.finalizeTimeRange(startMs, endMs);
  ...
  async processFileByFrame(startIndex, endIndex) {
    this.startProcessVideoTime = performance.now();
    [this.nb_samples, this.timeRangeStart, this.timeRangeEnd] = await this.sampleManager.finalizeSampleInIndex(startIndex, endIndex);
    this.uiManager.updateFrameCount(0, this.nb_samples);
    await this.processFile();
  }
  ...
  async setupDemuxer(file) {
    this.demuxer = new _videoDecoder_js__WEBPACK_IMPORTED_MODULE_2__.MP4Demuxer(file, {

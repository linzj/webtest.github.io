...
var length = width*height; var canvasnoisedata=new Uint8ClampedArray(length * 4);

setTimeout(function make_noise() {
var i=length, z=Math.random()*1024;
  while ( i-- ) {
      var n = noise(i%width+z,i/width+z,z);
      canvasnoisedata[i * 4] = n * 255;
      canvasnoisedata[i * 4 + 1] = n * 255;
      canvasnoisedata[i * 4 + 2] = n * 255;
      canvasnoisedata[i * 4 + 3] = 255;
  }
...
self.onmessage = function (e) {
  ...
  setup(o.width, o.height);
};
\`
  self.importScripts("data:," + encodeURI(src));
})()
`
...
var myworker = new Worker("data:," + encodeURI(src)), width=window.innerWidth, height=window.innerHeight, context=document.getElementById('canvas').getContext('2d');

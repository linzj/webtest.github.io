...
class App {
    constructor() {
        ...
        this.sessionManager = new SessionManager(this.storageManager, this.uiManager);
        this.fileManager = new FileManager(this.uiManager);
        this.chatInterface = new ChatInterface(this.uiManager);
        this.chatClient = new ChatClient(this.apiKeyManager, this.storageManager, this.uiManager);
    ...
    async initialize() {
        ...
        try {
            ...
            this.uiManager.updateStatus("Error: Database initialization failed. Chat history and custom models unavailable.", "error");
        ...
        if (this.apiKeyManager.isKeySet()) {
            this.uiManager.getElement('apiKeyInput').value = this.apiKeyManager.getKey();
            ...
            if (!this.chatClient.initializeGenAI()) {
             ...
             this.uiManager.updateStatus("API Key loaded. GenAI Client ready.", "success");

            // 4. Initialize Models (requires DB potentially)
            let initialModel = null;
            try {
                initialModel = await this.modelManager.initialize();
                 this.uiManager.updateStatus("API Key loaded. Models loaded.", "success");
            } catch (modelError) {
                 console.error("Error initializing models:", modelError);
                 this.uiManager.updateStatus("API Key loaded. (Error loading models)", "warning");
            }
            ...
            try {
                initialSessionId = await this.sessionManager.initialize();
                 this.uiManager.updateStatus("API Key loaded. Models loaded. Sessions loaded.", "success");
            } catch (sessionError) {
                 console.error("Error initializing sessions:", sessionError);
                 this.uiManager.updateStatus("API Key loaded. Models loaded. (Error loading sessions)", "warning");
            }
            ...
            if (initialModel && initialSessionId) {
                const chatReady = await this.chatClient.initializeChatSession(initialModel, initialSessionId);
                if (chatReady) {
                     this.uiManager.updateStatus(`Chat ready (${initialModel}).`, "success");
                } else {
                    // Status already updated by chatClient
                }
            } else {
                 this.uiManager.updateStatus("Chat not ready (missing model or session).", "warning");
            }
    ...
    async loadChatHistory(sessionId) {
        ...
        try {
            const messages = await this.storageManager.getRawMessagesForSession(sessionId);
            console.log(`Displaying ${messages.length} messages for session ${sessionId}.`);
            messages.forEach((msg) => {
                // Pass null for usageMetadata when loading from DB
                this.chatInterface.addMessage(msg.sender, msg.contentParts, null);
            });
        } catch (error) {
            console.error(`Error loading or displaying history for session ${sessionId}:`, error);
            this.chatInterface.addMessage('system', [{ text: `Error loading chat history for session ${sessionId}.` }]);
            this.uiManager.updateStatus(`Error loading history for session ${sessionId}.`, "error");
        }
    }
    ...
    async handleSendMessage() {
        ...
        if (!currentSessionId) {
             this.chatInterface.addMessage('system', [{ text: "Error: No active chat session selected." }]);
             return;
        }
        if (!this.chatClient.isChatReady()) {
             this.chatInterface.addMessage('system', [{ text: `Error: Chat not ready. Please check API key and model selection.` }]);
             return;
        }
        ...
        if (filesToSend.length > 0) {
             try {
                ...
                filesToSend.forEach((f) => {
                 ...
                 this.chatInterface.addMessage('system', [{ text: `Error processing files: ${fileError.message}` }]);
                 this.uiManager.updateStatus(`Error processing files: ${fileError.message}`, "error");
        ...
        try {
            ...
            await this.storageManager.saveMessage({ sender: "user", contentParts: apiMessageParts }, currentSessionId);
        } catch (saveError) {
        ...
        if (result.response) {
            ...
            // Save model response
            try {
                 await this.storageManager.saveMessage({ sender: "model", contentParts: responseParts }, currentSessionId);
            } catch (saveError) {
            ...
            this.chatInterface.addMessage('model', [{ text: `Error: ${result.error}` }]);
            this.uiManager.updateStatus(`Error sending message: ${result.error}`, "error");
        }
    }
    ...
    handleApiKeySave() {
        ...
        if (this.apiKeyManager.saveKey(newKey)) {
            ...
            if (this.chatClient.initializeGenAI()) {
                 this.uiManager.updateStatus("API Key valid. GenAI Client ready.", "success");
     ...
     */
     async reinitializeAppState() {
         ...
         if (currentModel && currentSession) {
             const chatReady = await this.chatClient.initializeChatSession(currentModel, currentSession);
             if (chatReady) {
    ...
    async handleModelChange(newModelName) {
        ...
        this.uiManager.updateStatus(`Switching to model ${newModelName}...`, "info");
        const sessionId = this.sessionManager.getCurrentSessionId();
        if (newModelName && sessionId) {
            const chatReady = await this.chatClient.initializeChatSession(newModelName, sessionId);
             if (chatReady) {
    ...
    async handleSessionChange(newSessionId, isNewSession) {
        console.log(`App received session change: ${newSessionId}${isNewSession ? ' (new)' : ''}`);
        ...
        if (modelName && newSessionId) {
            const chatReady = await this.chatClient.initializeChatSession(modelName, newSessionId);
             if (chatReady) {
     ...
     */
     async handleSearchToggle() {
         console.log("Google Search checkbox toggled. Re-initializing chat object...");
         const modelName = this.modelManager.getCurrentModel();
         const sessionId = this.sessionManager.getCurrentSessionId();
         if (modelName && sessionId) {
             // Re-initialize the chat session with the new tool setting
             const chatReady = await this.chatClient.reInitializeChatSession(sessionId);
             if (chatReady) {
                 const searchStatus = this.useGoogleSearchCheckbox?.checked ? "ON" : "OFF";
                 this.uiManager.updateStatus(`Chat ready (${modelName}). Search: ${searchStatus}`, "success");
             } else {
                  this.uiManager.updateStatus(`Failed to re-initialize chat for search toggle.`, "error");
             }
         } else {
             console.warn("Cannot re-initialize chat for search toggle: Missing model or session.");
         }
     }
    ...
    _setupEventListeners() {
        ...
        if (this.messageInput) {
         ...
         if (this.useGoogleSearchCheckbox) {
            this.useGoogleSearchCheckbox.addEventListener("change", () => this.handleSearchToggle());
        }

        // Manager Callbacks
        this.modelManager.setOnModelChange((newModel) => this.handleModelChange(newModel));
        this.sessionManager.setOnSessionChange((newSessionId, isNew) => this.handleSessionChange(newSessionId, isNew));
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    app.initialize().catch(error => {
        ...
        if (statusElement) {
            statusElement.textContent = "Critical Error: App failed to initialize. Check console.";

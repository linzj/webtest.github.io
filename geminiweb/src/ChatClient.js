...
export class ChatClient {
  ...
  initializeGenAI() {
    ...
    try {
      ...
      this.uiManager.updateStatus(
        `Error initializing GenAI: ${error.message}. Check key/network.`,
        "error"
      );
      return false;
    }
  ...
  async initializeChatSession(modelName, sessionId) {
    ...
    console.log(
      `Initializing chat session for model: ${modelName}, session: ${sessionId}`
    );
    ...
    try {
      ...
      let history = [];
      try {
        history = await this.storageManager.getFormattedHistoryForSession(
          sessionId
        );
        console.log(
          `Loaded ${history.length} history entries for session ${sessionId}.`
        );
      } catch (histError) {
        console.error(
      ...
      if (this.useGoogleSearchCheckbox?.checked) {
        ...
        console.log(
          "Google Search Retrieval tool enabled for this chat session."
        );
      } else {
        console.log(
          "Google Search Retrieval tool disabled for this chat session."
        );
      }
      ...
      console.error(
        `Error initializing chat session for model ${modelName}:`,
        error
      );
      this.uiManager.updateStatus(
        `Error initializing model ${modelName}. Check model name/permissions.`,
        "error"
      );
      this.chat = null;
      return false;
  ...
  async sendMessage(messageParts) {
    ...
    console.log(
      `Sending ${messageParts.length} parts to model ${this.currentModelName}...`
    );
  ...
  async reInitializeChatSession(sessionId) {
    ...
    console.log(
      `Re-initializing chat session for model ${this.currentModelName}, session ${sessionId}...`
    );
    return this.initializeChatSession(this.currentModelName, sessionId);
  }

  /**
   * Sends a message using a specific model, history, and parts, without using the main `this.chat` instance.
   * Useful for operations like retrying a message with truncated history.
   * @param {string} modelName - The generative model name to use.
   * @param {Array<{role: string, parts: Array<object>}>} history - The specific history context to use.
   * @param {Array<object>} messageParts - An array of parts (text or inlineData) for the message to send.
   * @param {boolean} useGoogleSearch - Whether to enable the Google Search tool for this specific call.
   * @returns {Promise<{response: object, error?: string}>} An object containing the API response or an error message.
   */
  async sendMessageWithHistory(
    ...
    if (!this.genAI) {
      console.error(
        "GenAI client not initialized. Cannot send message with history."
      );
      return { error: "GenAI client not initialized." };
    }
    if (!modelName) {
      console.error("Model name required for sendMessageWithHistory.");
      return { error: "Model name required." };
    }
    if (!messageParts || messageParts.length === 0) {
      console.warn("Attempted to send an empty message with history.");
      return { error: "Cannot send empty message." };
    }

    console.log(
      `Sending message with specific history (${history.length} entries) to model ${modelName}...`
    );
    ...
    try {
      ...
      // 2. Prepare StartChatParams (History and Tools)
      const startChatParams = {
      ...
      if (useGoogleSearch) {
        ...
        startChatParams.tools.push(googleSearchTool);
        console.log(
          "Google Search Retrieval tool enabled for this specific call."
        );
      }

      // 3. Start a TEMPORARY Chat Session
      const tempChat = model.startChat(startChatParams);

      // 4. Send the Message using the temporary chat
      const result = await tempChat.sendMessage(messageParts);
      console.log("Message sent successfully using temporary chat.");

      return { response: result.response };
    } catch (error) {
      console.error(
        "Error sending message with history via Gemini API:",
        error
      );
      return {
        error: error.message || "Unknown error sending message with history.",
      };
    }
  }
}

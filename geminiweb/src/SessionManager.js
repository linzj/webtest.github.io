...
export class SessionManager {
        ...
        try {
            ...
            if (this.sessions.length === 0) {
                ...
                if (sessionToLoad) {
                    ...
                    this.uiManager.selectDropdownValue(this.sessionSelector, this.currentSessionId);
                    localStorage.setItem(LAST_SESSION_KEY, this.currentSessionId);
                    console.log("Sessions loaded. Initial session:", this.currentSessionId);
                } else {
                     console.error("Could not determine initial session ID even after loading/creating.");
                     this.currentSessionId = null;
                }
            }
        } catch (error) {
    ...
    async createNewSession(isInitial = false) {
        try {
            ...
            this.uiManager.updateStatus(`Error creating session: ${error.message}`, "error");
    ...
    async switchSession(sessionId) {
        ...
        if (!this.sessions.some(s => s.id === newSessionId)) {
            console.warn(`Attempted to switch to non-existent session ID: ${newSessionId}`);
    ...
    async deleteCurrentSession() {
        ...
        const sessionName = this.uiManager.getSelectedDropdownText(this.sessionSelector) || `Session ${sessionIdToDelete}`;
        if (!confirm(`Are you sure you want to delete "${sessionName}" and all its messages? This cannot be undone.`)) {
        ...
        try {
            ...
            this.uiManager.removeDropdownOption(this.sessionSelector, sessionIdToDelete);
            ...
            if (this.uiManager.getDropdownLength(this.sessionSelector) > 0) {
                ...
                nextSessionId = parseInt(this.uiManager.getFirstDropdownValue(this.sessionSelector), 10);
            } else {
            ...
            this.uiManager.updateStatus(`Error deleting session: ${error.message}`, "error");
        }
    }
    ...
    populateSessionOption(session, prepend = false) {
        const sessionName = session.name || `Session ${new Date(session.createdAt).toLocaleString()}`;
        this.uiManager.addDropdownOption(this.sessionSelector, sessionName, session.id, prepend);
    }
